...
public class DecorationLayerRenderer {
    public static void addDecorations(Tile decoration, int xTile, int yTile, int x, int y) {
        int numberOfTextures = decoration.getNumberOfDifferentTextures();
        if (numberOfTextures == 1)
            SpriteBatchUtils.getInstance().drawPlain(decoration.getTexture(AsteraniaMain.INSTANCE.getAssetManager().get(Atlases.DECORATION_ATLAS_LOCATION, TextureAtlas.class)), xTile, yTile, DisplayConfig.TILE_SIZE, DisplayConfig.TILE_SIZE);
        else {
            ...
            SpriteBatchUtils.getInstance().drawPlain(decorationTexture, xTile, yTile, DisplayConfig.TILE_SIZE, DisplayConfig.TILE_SIZE);
        }
    }
}
